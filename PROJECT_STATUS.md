# Project Status: Kindle Acceptance Passed (Merge Pending)

更新時間：2026-07-16（Asia/Taipei）

## 結論

Scope reset 的產品變更已由 PR #23 合併至 `main`；production server 已部署，
DP75SDI Kindle 候選 runtime 已通過真機驗收，但本文件所在 acceptance branch 尚待
commit、push、CI 與 merge。目前 release 狀態是：

```text
server deployed, Kindle candidate accepted, merge pending, macOS beta
```

伺服器與 collector 已完成交付，Kindle candidate 已完成可自動驗證及真機驗收；只有在
acceptance branch 合併並重新確認 CI/deployment metadata 後，才能說 canonical `main`
包含本次修正。portable macOS collector 仍是 Beta，因為 personal Mac acceptance 依使用者
指示暫緩。不要重啟已退役的 2026-07-13/14 hardening execution plans，也不要把尚未提供的
Mac session 誤判成需要更多模擬框架。
...
1. 合併 Kindle acceptance branch 前，確認 diff、CI 與 production deployment SHA；runtime-only
   變更仍須明確區分 GitHub merge 與 Kindle USB 同步。
