...
test('provider cards clamp progress values to the e-ink progress bar range', () => {
  const cards = getProviderCards({
    CLAUDE_STATUS_VALUE: '120%',
    CLAUDE_PROGRESS_VALUE: '250',
    OPENAI_STATUS_VALUE: '-5%',
    OPENAI_PROGRESS_VALUE: '-5',
  });

  assert.equal(cards[0].progress, 100);
  assert.equal(cards[1].progress, 0);
});

test('provider env template lists the display values and optional progress overrides', () => {
